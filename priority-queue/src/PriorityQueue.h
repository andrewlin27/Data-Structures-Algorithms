...
private:
    ...
    void downheap(size_type index) {
        ...
        size_type greaterChild = left_child(index);

        // if right child exists and is greater than left child
        if (right_child(index) < size() && comp(c.at(left_child(index)), c.at(right_child(index)))) {
            greaterChild = right_child(index);
        }

        if (comp(c.at(index), c.at(greaterChild))) {
            std::swap(c.at(index), c.at(greaterChild));
        }
        downheap(greaterChild);
    }
...
public:
     ...
     */
    void pop() {
        //move last element to root, delete last element, call downheap. 
