...
std::vector<DatumGT> generate_file_data(Typegen & t, DatumGT::WantedSanitation sanitation) {
    size_t points = t.range<size_t>(0, 200);
    std::vector<DatumGT> data(points);
    for(size_t i = 0; i < points; i++)
		data[i] = DatumGT::generate(t, sanitation);
    return data;
}


DatumGT DatumGT::generate(Typegen & t, DatumGT::WantedSanitation sanitation) {
    ...
    switch(sanitation) {
        case DatumGT::WantedSanitation::GOOD: gt.bad = false; break;
        case DatumGT::WantedSanitation::BAD:  gt.bad = true; break;
        default: gt.bad = t.get<bool>(0.2);
    }
