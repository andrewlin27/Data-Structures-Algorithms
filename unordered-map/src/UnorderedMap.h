...
private:
    ...
    HashNode*& _find(size_type code, size_type bucket, const Key & key) {
        ...
        while (*curr) {
            if (_hash((*curr)->val.first) == code && _equal((*curr)->val.first, key)) {
                return *curr;
            }
            curr = &(*curr)->next;
        }
    ...
    HashNode * _insert_into_bucket(size_type bucket, value_type && value) {
        ...
        HashNode* node = new HashNode(std::move(value), bucketHead);
        if (!_head || bucket <= _bucket(_head->val)) {
            _head = node;
        }
...
public:
    ...
    UnorderedMap(UnorderedMap && other) : _bucket_count(other._bucket_count), _hash(other._hash), _equal(other._equal), _size(other._size), _head(other._head), _buckets(other._buckets) {
        other._hash = Hash{};
    ...
    std::pair<iterator, bool> insert(value_type && value) {
        ...
        size_t bucket = _bucket(value);
        HashNode* element = _find(code, bucket, value.first);
        if (element) {
            val = std::make_pair(iterator(this, element), false);
        }
        else {
            element = _insert_into_bucket(_bucket(value.first), std::move(value));
            val = std::make_pair(iterator(this, element), true);
        }
        return val;
    }
    ...
    std::pair<iterator, bool> insert(const value_type & value) {
        ...
        if (element) {
            ...
            element = _insert_into_bucket(_bucket(copy.first), std::move(copy));
            val = std::make_pair(iterator(this, element), true);
        }
        return val;
    }
    ...
    T& operator[](const Key & key) {
        ...
        if (node) {
            ...
            auto newPair = std::make_pair(key, mapped_type{});
            return insert(newPair).first->second;
        }
    }
    ...
    iterator erase(iterator pos) {
        ...
        HashNode* delNode = node;
        iterator del = iterator(this, delNode);
        del++;
        node = node->next;
        delete delNode;
        _size--;
